...
reads_dict = {}
undef_reads = {}
with open("/mnt/md0/oe_full_length/output/OSNfulllength/isoquant_bam/OUT/OUT.read_assignments.tsv", "r") as fs:
    line = fs.readline()
    line = fs.readline()
    line = fs.readline()
    line = fs.readline()
...
with open("/mnt/md0/oe_full_length/output/OSNfulllength/OSNfulllength.read_summary.tsv", "r") as fs:
    line = fs.readline()
    line = fs.readline()
    while line:
        line = line.strip().split("\t")
...
res = open("/mnt/md0/oe_full_length/output/OSNfulllength/isoquant_bam/OUT/OUT.read_assignments_fix.tsv", "w")
with open("/mnt/md0/oe_full_length/output/OSNfulllength/OSNfulllength.read_summary.tsv", "r") as fs: 
    line = fs.readline()
    line = fs.readline()
    while line:
...
cells = {}
cell_id = 0
with open("/mnt/md0/oe_full_length/output/OSNfulllength/osn_bc_info.tsv", "r") as fs:
    line = fs.readline()
    while line:
        line = line.strip().split("\t")
...
with open("/mnt/md0/oe_full_length/output/OSNfulllength/isoquant_bam/OUT/OUT.read_assignments_fix.tsv", "r") as fs:
    line = fs.readline()
    while line:
        line = line.strip().split("\t")
        bc = line[9] + "-1"
...
with tables.open_file("/mnt/md0/oe_full_length/output/OSNfulllength/osn_junction_info.h5", "w", title="") as fs:
...
with tables.open_file("/mnt/md0/oe_full_length/output/OSNfulllength/osn_gene_info.h5", "w", title="") as fs:
    group = fs.create_group("/", "matrix", "Datasets of count")
    feature_group = fs.create_group(group, "features", "Genes and other features measured")
    for k, v in features.items():
        r = fs.create_array(feature_group, k, np.array(v))
...
with open("/mnt/md0/oe_full_length/output/OSNfulllength/osn_gene_len.tsv", "w") as fs:
...
utr3_matrix = sparse.csc_matrix((np.array(utr3_data), (np.array(count_row), np.array(count_col))), shape=(len(features["id"]), len(cells)))
utr5_matrix = sparse.csc_matrix((np.array(utr5_data), (np.array(count_row), np.array(count_col))), shape=(len(features["id"]), len(cells)))
with tables.open_file("/mnt/md0/oe_full_length/output/OSNfulllength/osn_trans_ass.h5", "w", title="") as fs:
    group = fs.create_group("/", "matrix", "Datasets of count")
    feature_group = fs.create_group(group, "features", "Genes and other features measured")
    for k, v in features.items():
        r = fs.create_array(feature_group, k, np.array(v))
...
with open("/mnt/md0/oe_full_length/output/OSNfulllength/osn_iso_len.tsv", "w") as fs:
...
with open("/mnt/md0/oe_full_length/output/OSNfulllength/osn_trans_len.tsv", "w") as fs:
    r = fs.write("\t".join(["type", "trans", "utr3", "utr5", "len", "r3", "r5", "rlen"])+"\n")
    for k, v in trans.items():
        for t, u in v.items():
            for x in u.values():
...
with open("/mnt/md0/oe_full_length/output/OSNfulllength/osn_gene_iso.tsv", "w") as fs:
...
samfile = "/mnt/md0/oe_full_length/output/OSNfulllength/OSNfulllength.tagged.bam"
...
with pysam.AlignmentFile(samfile) as fs:
    res = pysam.AlignmentFile("/mnt/md0/oe_full_length/output/OSNfulllength/OSNfulllength.tagged_fix.bam", "wb", header=fs.header)
...
r = os.system("samtools sort -@8 OSNfulllength.tagged_fix.bam -o OSNfulllength.tagged_fix.sort.bam")
r = os.system("samtools index OSNfulllength.tagged_fix.sort.bam")
r = os.system("rm OSNfulllength.tagged_fix.bam")
r = os.system("scp OSNfulllength.tagged_fix.sort.bam* xuyc7@202.116.90.56:/usr/share/nginx/html/jbrowse2/usr_data/")
...
with open("iso_trans_test_aa_cal.tsv", "w") as fs:
    ...
    for k, v in terms.items():
        t = [k]
        t.append(str(round(100-v[55:84].count("-")*100/29, 2)))
        t.append(str(round(100-v[90:120].count("-")*100/30, 2)))
        t.append(str(round(100-v[130:164].count("-")*100/34, 2)))
        t.append(str(round(100-v[177:202].count("-")*100/25, 2)))
        t.append(str(round(100-v[224:260].count("-")*100/36, 2)))
        t.append(str(round(100-v[283:304].count("-")*100/21, 2)))
        t.append(str(round(100-v[314:337].count("-")*100/23, 2)))
        r = fs.write("\t".join(t)+"\n")


cells = {}
with open("/home/cbh/work/oe_fl_re/osn_olfr_info.tsv", "r") as fs:
    line = fs.readline()
    line = fs.readline()
    while line:
        line = line.strip().split("\t")
...
fs = pysam.AlignmentFile("/mnt/md0/oe_full_length/output/OSNfulllength/OSNfulllength.tagged_fix.sort.bam")
res_fn = "/mnt/md0/oe_full_length/output/OSNfulllength/OSNfulllength.tagged_olfr.bam"
...
utr3_matrix = sparse.csc_matrix((np.array(utr3_data), (np.array(count_row), np.array(count_col))), shape=(len(features["id"]), len(cells)))
utr5_matrix = sparse.csc_matrix((np.array(utr5_data), (np.array(count_row), np.array(count_col))), shape=(len(features["id"]), len(cells)))
with tables.open_file("/mnt/md0/oe_full_length/output/OSNfulllength/osn_gene_ass.h5", "w", title="") as fs:
    group = fs.create_group("/", "matrix", "Datasets of count")
    feature_group = fs.create_group(group, "features", "Genes and other features measured")
    for k, v in features.items():
        r = fs.create_array(feature_group, k, np.array(v))
...
fs = pysam.AlignmentFile("/mnt/md0/oe_full_length/output/OSNfulllength/tagged.bam")
bam_list = {}
for x in types:
    bam_list[x] = pysam.AlignmentFile(f"/mnt/md0/oe_full_length/output/OSNfulllength/tagged_{x}.bam", "wb", header=fs.header)
...
fs = pysam.AlignmentFile("/mnt/md0/oe_full_length/output/OSNfulllength/tagged.bam")
res = pysam.AlignmentFile(f"/mnt/md0/oe_full_length/output/OSNfulllength/tagged_neuron_hdac9_raw.bam", "wb", header=fs.header)
